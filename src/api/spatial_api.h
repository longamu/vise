...
class API : public absAPI {
 ...
 API(spatialRetriever const &aSpatialRetriever_obj,
  ...
  std::string
    getReply( boost::property_tree::ptree &pt, std::string const &request );

 private:
  void ExecuteQueryForAnnotatedResults( query &query_obj, uint32_t startFrom, uint32_t numberToReturn, std::string &output );
  ...
  void
    ReturnAnnotatedResults( std::vector<indScorePair> const &queryRes, std::map<uint32_t,homography> const *Hs, uint32_t startFrom, uint32_t numberToReturn, std::string &output, query &query_obj );
