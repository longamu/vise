...
def get(
    ...
    if datasetOpt['search_engine_config_filename']:
      config.read( datasetOpt['search_engine_config_filename'] );
    else:
...
if __name__=='__main__':
    ...
    if len(sys.argv)>5:
        ...
        for i in range(2, len(sys.argv), 4):
            datasetOpts.append( {'dsetname': sys.argv[i], 'APIport': int(sys.argv[i+1]), 'enableUpload': sys.argv[i+2]=='u', 'search_engine_config_filename': sys.argv[i+3]} );
